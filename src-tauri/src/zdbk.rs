...
pub async fn login_zdbk(state: &AppState) -> Result<(), String> {
    ...
    let res = client.get(cas_url)
        .header("Cookie", format!("iPlanetDirectoryPro={}", cookie_val))
        .send().await.map_err(|e| format!("CAS请求失败: {}", e))?;
    ...
    let res2 = client.get(&location)
        .send().await.map_err(|e| format!("教务网重定向失败: {}", e))?;
...
pub async fn get_transcript(state: &AppState) -> Result<Vec<Value>, String> {
    ...
    let res = client.post(url)
        ...
        .send().await.map_err(|e| format!("成绩查询失败: {}", e))?;

    let text = res.text().await.map_err(|e| format!("读取成绩响应失败: {}", e))?;
    ...
    if let (Some(s), Some(e)) = (start, end) {
        if s < e {
            ...
            let grades: Vec<Value> = serde_json::from_str(json_str).map_err(|e| format!("JSON解析失败: {}", e))?;
            return Ok(grades);
        }
    }
    
    Err(format!("无法解析成绩数据，响应截断: {}", &text[..std::cmp::min(200, text.len())]))
}
...
pub async fn get_major_grades(state: &AppState) -> Result<Vec<Value>, String> {
    ...
    let res = client.post(url)
        .header("Cookie", &cookies)
        .send().await.map_err(|e| format!("主修成绩查询失败: {}", e))?;

    let text = res.text().await.map_err(|e| format!("读取主修成绩响应失败: {}", e))?;
    ...
    Err(format!("无法解析主修成绩数据: {}", &text[..std::cmp::min(200, text.len())]))
}

/// Fetch timetable for a given academic year and semester.
/// year: e.g. "2024" semester: "1" for 秋冬, "2" for 春夏
pub async fn get_timetable(state: &AppState, year: &str, semester: &str) -> Result<Vec<Value>, String> {
    let client = &state.client;
    let jsid = state.zdbk_jsessionid.lock().await;
    let rt = state.zdbk_route.lock().await;
    let cookies = zdbk_cookies(
    ...
    let res = client.post(url)
        ...
        .body(form_body)
        .send().await.map_err(|e| format!("课表查询失败: {}", e))?;

    let text = res.text().await.map_err(|e| format!("读取课表响应失败: {}", e))?;
    ...
    if let (Some(s), Some(e)) = (start, end) {
        if s < e {
            let json_str = &text[s..e];
            let sessions: Vec<Value> = serde_json::from_str(json_str).map_err(|e| format!("JSON解析失败: {}", e))?;
            return Ok(sessions);
        }
    }
    
    Err(format!("无法解析课表数据: {}", &text[..std::cmp::min(200, text.len())]))
}
...
pub async fn get_exams(state: &AppState) -> Result<Vec<Value>, String> {
    ...
    let res = client.post(url)
        .header("Cookie", &cookies)
        .send().await.map_err(|e| format!("考试查询失败: {}", e))?;

    let text = res.text().await.map_err(|e| format!("读取考试响应失败: {}", e))?;
    ...
    if let (Some(s), Some(e)) = (start, end) {
        if s < e {
            let json_str = &text[s..e];
            let exams: Vec<Value> = serde_json::from_str(json_str).map_err(|e| format!("JSON解析失败: {}", e))?;
            return Ok(exams);
        }
    }
    
    Err(format!("无法解析考试数据: {}", &text[..std::cmp::min(200, text.len())]))
}
...
pub async fn get_practice_scores(state: &AppState) -> Result<PracticeScores, String> {
    ...
    let res = client.get(&url)
        .header("Cookie", &cookies)
        .send().await.map_err(|e| format!("实践分查询失败: {}", e))?;

    let html = res.text().await.map_err(|e| format!("读取实践分响应失败: {}", e))?;

    let mut scores = PracticeScores { pt2: 0.0, pt3: 0.0, pt4: 0.0 };
...
fn extract_practice_score(html: &str, class_name: &str) -> Option<f64> {
    let pattern = format!(r#"<td[^>]*>{}</td>.*?<td[^>]*>([0-9.]+)</td>"#, regex::escape(class_name));
    let re = regex::Regex::new(&pattern).ok()?;
    re.captures(html).and_then(|caps| caps[1].parse::<f64>().ok())
}
